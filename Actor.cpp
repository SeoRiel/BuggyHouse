...
Actor::Actor(D2DFramework* pFramework, LPCWSTR filename)
	: mpFramework{ pFramework }, mpBitmap{}, mX{}, mY{}, mOpacity{1.0f}
{
...
Actor::Actor(D2DFramework* pFramework, LPCWSTR filename, float x, float y, float opacity)
	: Actor(pFramework, filename) // -> 대리 생성자 : 다른 생성자로 멤버 변수 초기화
{
	mX = x;
	mY = y;
	mOpacity = opacity;
...
void Actor::Draw(float x, float y, float opacity)
{
	...
	mpFramework->GetRenderTarget()->DrawBitmap(mpBitmap, rect, opacity);
