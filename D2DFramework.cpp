...
HRESULT D2DFramework::InitializeD2D()
{
	...
	hr = D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, mspD2D1Factory.GetAddressOf());
	ThrowIfFailed(hr);
...
HRESULT D2DFramework::Initialize(HINSTANCE hInstance, LPCWSTR title, UINT width, UINT height)
{
	...
	hr = BitmapManager::Instance().Initiailze(mspRenderTarget.Get());
	ThrowIfFailed(hr, "Failed to create Bitmap Manager");
